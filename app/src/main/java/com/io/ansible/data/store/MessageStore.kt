 ...
 */
class MessageStore(private val messageDao: MessageDao, private val messageWithContactDao: MessageWithContactDao) {
    init {
    ...
    fun observeMessages(threadId: String): Flowable<List<MessageWithContact>> {
        return messageWithContactDao.get(threadId)
    }
    ...
    fun sendMessage(senderId: String, receiverId: String, messageBody: String) {
        ...
        val messageEntity = MessageEntity(receiverId, senderId, System.currentTimeMillis(), MessageEntity.DIRECTION_OUTGOING, messageBody)
    ...
    private fun onChatReceived(senderJid: EntityBareJid, message: Message) {
        ...
        val messageEntity = MessageEntity(senderId, senderId, System.currentTimeMillis(), MessageEntity.DIRECTION_INCOMING, message.body)
        insertMessage(messageEntity)
    }
